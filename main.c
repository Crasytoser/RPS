...
static const uint32_t images[] = {RESOURCE_ID_paper,RESOURCE_ID_rock,RESOURCE_ID_scissors,}; /* создаем массив с номерами картинок */
...
void timer_call() /* эта функция вызывается при срабатываниии таймера и при первом запуске перебора */
{
    ...
    image_layer = bitmap_layer_create(GRect(32, 44, 80, 80));  /* создаем слой в центре экрана */
    layer_add_child(window_get_root_layer(window), bitmap_layer_get_layer(image_layer)); /* делаем его дочерним */
    image = gbitmap_create_with_resource(images[rand() % 3]); /* загружаем в память случайную картинку из подключенных ресурсов */
...
int main(void)
{
    ...
    text_layer_set_text(text_layer, "Встряхните часы");  /* показываем сообщение при запуске */
